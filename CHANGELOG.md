...
- **进度条延迟显示**: 
  - 进度条现在在检测到缺失范围后才显示
  - 避免在打印缺失范围信息前显示空进度条(0.0%)
  - 改善了CLI的视觉体验和信息流
  
- **退出时不重复显示进度条**:
  - 用户按 Ctrl+C 中断下载时,进度条立即停止更新
  - 避免在保存进度时重复打印进度条
  - 提供清晰的 "已取消" 提示信息
...
- **技术实现**:
  - 使用 Rich Progress 的 `visible=False` 参数初始化进度条
  - 在首次调用 `update_progress` 回调时才显示进度条
  - 捕获 `KeyboardInterrupt` 时设置 `progress_stopped` 标志
  - 调用 `progress.stop()` 立即停止进度条渲染
  - 确保缺失范围信息先于进度条显示
