        ...
        for indicator in indicators:
            try:
                ...
                if indicator_name == 'MA':
                    ...
                    df[indicator] = self.indicator_calc.ma(df['close'], period)
                
                elif indicator_name == 'EMA':
                    period = int(parts[1]) if len(parts) > 1 else 12
                    df[indicator] = self.indicator_calc.ema(df['close'], period)
                
                elif indicator_name == 'RSI':
                    period = int(parts[1]) if len(parts) > 1 else 14
                    df[indicator] = self.indicator_calc.rsi(df['close'], period)
                    ...
                    macd, signal, hist = self.indicator_calc.macd(df['close'])
                    df['MACD'] = macd
